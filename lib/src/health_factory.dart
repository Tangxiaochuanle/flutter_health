...
class HealthFactory {
  ...
  Future<bool> writeHealthData(
    ...
    if (type == HealthDataType.WORKOUT) throw ArgumentError("Adding workouts should be done using the writeWorkoutData method.");
    ...
    if ({
        ...
        _platformType == PlatformType.IOS) throw ArgumentError("$type - iOS doesnt support writing this data type in HealthKit");
  ...
  Future<bool> writeBloodOxygen(double saturation, DateTime startTime, DateTime endTime, {double flowRate = 0.0}) async {
    if (startTime.isAfter(endTime)) throw ArgumentError("startTime must be equal or earlier than endTime");
    bool? success;
  ...
  Future<bool> writeAudiogram(List<double> frequencies, List<double> leftEarSensitivities, List<double> rightEarSensitivities,
  ...
  Future<List<HealthDataPoint>> getHealthDataFromTypes(DateTime startTime, DateTime endTime, List<HealthDataType> types) async {
  ...
  Future<int?> getTotalStepsInInterval(
    ...
    final args = <String, dynamic>{'startTime': startTime.millisecondsSinceEpoch, 'endTime': endTime.millisecondsSinceEpoch};
